...
class _ProductDetailsScreenState extends State<ProductDetailsScreen> {
  ...
  void initState() {
    ...
    CategoriesCubit.get(context).fetchRelatedProducts(
        catId: widget.product.categories == null ? 0 : widget.product.categories[0]['id'],
        page: 1,
        perPage: 15);
                        ...
                        InkWell(
                          ...
                          onTap: () async {
                            await FlutterShare.share(
                                ...
                                linkUrl:widget.product.permalink ?? "https://alshiaka.com/en/product/rethobe-boys-2263-white/",
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: InkWell(
                              ...
                              child: SvgPicture.asset("${AppUI.iconPath}face.svg"),
                            ),
                          ),
                          ...
                          SizedBox(
                            ...
                            child: InkWell(
                              ...
                              child: SvgPicture.asset("${AppUI.iconPath}twitter.svg",),
                            ),
                          ),
                          SizedBox(width: SizeConfig.padding / 4,),
                          SizedBox(
                  ...
                  SizedBox(
                      ...
                      child: CustomText(
                        text: widget.product.description!.length < 3
                            ...
                            : widget.product.description!
                                .substring(3, widget.product.description!.length - 5),
                        color: AppUI.blackColor,
                        fontSize: 16,
                      )),
                  ...
                  BlocBuilder<CategoriesCubit, CategoriesState>(
                    ...
                    return SizedBox(
                      ...
                      child: ListView(
                        ...
                        children: List.generate(widget.product.attributes!.length,
                            (mainIndex) {
                          return Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Row(
                                children: [
                                  if (widget.product.attributes![mainIndex].options !=
                                      null)
                                    CustomText(
                                      text:
                                  ...
                                  const Spacer(),
                                  if (widget.product.attributes![mainIndex].name ==
                                      "Size")
                                    InkWell(
                                        onTap: () {
                                          AppUtil.mainNavigator(
                                              ...
                                              SizeGuideScreen(
                                                title: widget.product.name,
                                                options: widget.product.attributes![mainIndex]
                                                        .options ??
                                                    [
                              ...
                              SizedBox(
                                ...
                                child: ListView(
                                  ...
                                  children: List.generate(
                                      widget.product.attributes![mainIndex].options !=
                                              null
                                      ...
                                      children: [
                                        CustomCard(
                                          onTap: () {
                                            ...
                                            if (widget.product.attributes!.length >=
                                                2) {
                                              for (var element
                                                  in cubit!.variations) {
                                                int y = 0;
                                          ...
                                          radius: 10,
                                          padding: 0.0,
                                          border: cubit!.selectedAttributeIndex[
                                                      mainIndex]['index'] ==
                                                  index
                                              ? AppUI.mainColor
                                              : null,
                                          ...
                                          child: Padding(
                                            padding: const EdgeInsets.symmetric(
                                                horizontal: 20),
                                            child: CustomText(
                                                text: widget.product.attributes![
                                                                mainIndex]
                                                            .options !=
                                                        null
                                                    ? widget.product.attributes![mainIndex]
            ...
            return CustomCard(
              ...
              child: CustomButton(
                ...
                onPressed: () async {
                  ...
                  List variant =
                      await cubit.getVariationId(cubit.variations, context);
                  if (variant[0] == -1) {
                  ...
                  if (variant[0] != 0) {
                    for (var element in cubit.variations) {
                      if (element.id == variant[0]) {
                        ...
                        element.mainProductId = widget.product.id.toString();
                        if (!mounted) return;
                        cubit.addToCart(context, element, slugs: variant[1]);
                      }
                    }
                  } else {
                    bool exists = await cubit.fetchItemInCart(widget.product.id);
                    if (!exists) {
                      widget.product.qty = 1;
                    }
                    widget.product.mainProductId = widget.product.id.toString();
                    if (!mounted) return;
  ...
  fetchAttributes() async {
    print('fvnkfjddfjfjn ${widget.product.id}');
    for (int i = 0; i < widget.product.attributes!.length; i++) {
      optionVisibility.add([]);
      for (var element in widget.product.attributes![i].options!) {
        if (i == 0) {
    ...
    await CategoriesCubit.get(context).fetchProductAttributes(widget.product);
    if (!mounted) return false;
    await CategoriesCubit.get(context).fetchProductVariations(widget.product.id);
    if (widget.product.attributes!.length >= 2) {
      for (var element in cubit!.variations) {
        int y = 0;
        for (var element2 in element.attributes!) {
          if (y > 0) {
            for (int i = 0; i < widget.product.attributes![1].options!.length; i++) {
              if (StringSimilarity.compareTwoStrings(
                          element2.option!.toLowerCase(),
                          widget.product.attributes![1].options![i].toLowerCase()) >
                      0.4 &&
                  element2.option!.length >=
                      widget.product.attributes![1].options![i].length &&
                  cubit!.selectedAttributeIndex[0]['name'] ==
                      element.attributes![y - 1].option) {
